...
| B6 | Accessibility audit — `axe-core` wired in (this app's first automated pass ever), found and fixed real WCAG AA failures affecting all 20 terminal screens (`--t-faint` contrast, missing `<main>` landmarks, the signature toggle's opacity); real screen-reader-software pass still open | **done** |
...
| 0.2 | **Two devices, one relay** | **narrowed 2026-09-02** | Was *"peer presence passes unit tests and has never crossed a relay."* The local-relay suite closed the client half; what it left open, in its own words, was whether `relay.damus.io` behaves. **`navcom-relay-check` now answers that**, and the answer is that both shipped defaults pass all five claims — a write is accepted, an **ephemeral event is delivered**, a tag filter actually filters, and replaceable and addressable events are stored and returned. That last-but-two mattered most: `Distress` is ephemeral, relays are not obliged to forward ephemeral events, and had one declined, `Distress` would never have left the phone with **the entire suite still green**. **What remains is the genuinely human half** — two phones, two networks, two people, on hardware, which no script can stand in for |
...
| 2.5 | ~~A way for a paged human to say *"I have this"* from their phone~~ | **done** | `distress-ack` was a defined signal with a ten-second budget, the executor accepted it, the roster could identify who sent it — and **no client sent one**. The service worker's own comment recorded the consequence: an earlier notification said *"open the terminal and acknowledge"* and there was no such control, so the text was changed to point at the console instead. Now the id travels with the page (`{{distress}}` in an operator's own command template, `navcom-push --distress`, an encrypted payload, `/terminal/?ack=<id>`) and one tap sends it. **The id has to travel that way**: `20911` is ephemeral, so a relay forwards it to whoever is subscribed at that instant and stores nothing — a phone that was asleep finds the event gone, and an event id being public does not help when there is nothing left to read it from. Found while building it: `notificationclick` focused an already-open terminal tab and **discarded the URL**, so this would have worked only for somebody with no tab open — the opposite of who is on-call. Six tests; breaking the explicit-tap rule fails three of them, which is the guard for *"someone whose phone buzzed is not someone who woke up"* |
...
**Everything in this milestone that code can do is now done.** 2.5 was the last of it. What
remains is one config entry and a proven channel (2.1), and one real phone receiving one real
page (2.3) — and until those, *"a Distress raises a real human"* is untested rather than false.
...
| 9.6 | **A restore drill** | **human**, and now checkable | The paging drill proves the pager works. Nothing proves a *second person* can stand a Watchtower up from the docs. **It will probably fail the first time, and that is the finding** — but it is only a finding if somebody can see it, and until now they could not. The escalation half had `--check` and `--drill` from the start; the daemon half had **no flags at all**, so a Stationkeeper could prove they were able to wake a person and had no way to prove an operator could *see* their watch. `watchtower-daemon --check` now answers that with the Field Terminal's own filter and reader, names which of the four Darks it is because they have four different fixes, and exits non-zero so it can be a cron line. Still human: the drill needs a second person to attempt it |
| 9.7 | More than one on-call, and a way to hand it over | **human** | 2.1 gets to one; this gets past one |
| 9.8 | The log reviewer | **human**, and now performable | Named in `CLAUDE.md` as a role that *"cannot be the agent or verification is theatre"*, costing *"minutes per week"*. **Nobody took it, and the tooling was the reason:** the job as it actually stood was `ssh` to the box and read a JSONL file by eye — which is not minutes, and which nobody can do wrong-way-up at 11pm. `navcom-escalation --review` is that week on one screen: the last drill and who answered it, every escalation **listed with its date rather than counted**, whether the log still verifies, who is on call, and a closing **NEEDS A LOOK** that on a good week says nothing does. It exits non-zero only when that section has something in it, so it can be a weekly cron that stays quiet. Written before the reviewer exists on the same argument `--check` was: **"minutes per week" is a claim the software has to make true before anybody can accept the job.** Still human — nothing here reads it |
...
| 9.10 | ~~A Stationkeeper onboarding doc~~ | **done** | [`watch/stationkeeper.md`](watch/stationkeeper.md) — the commitment, before the setup steps: uptime you're honest about rather than promise, drills that fail until somebody is on-call, no key recovery story yet, being the highest-privilege position in the system. Points at the example TOML configs as the real starting point until 9.4 shortens it |
| 9.12 | A per-op recap, for posting where the community already is | **deferred** | Trigger: *somebody wants to post one.* `propagation.md` §2 specifies it and `visibility.md` has the switch with no code behind it. **There is no format decision to make** — plain text is what a forum takes, and the contribution record already produces it; what is missing is the *scope*, one night rather than the career. Anything more structured would be designing against Herocore's submission form, which is members-only and cannot be seen from here |
| 9.11 | ~~Point at the community's own hubs, and away from the squatted ones~~ | **done** | Not a NavCom single-point-of-failure but the community's, which is the same milestone's concern one level out: RLSH properties decay and get taken over while nobody is watching. `therlsh.forumotion.com` now redirects to a squatter and `superheroesanonymous.org` resolves nowhere — **both found by trying the links, not by reading about them**. The list is typed data (`web/src/lib/community.ts`) rendered on `/about/`, not prose, because the first version was a page about link rot that rotted: it shipped a live entry with no link and two archive links on plaintext `http://`. `community.test.ts` asserts against the **built HTML** that no page links a blocklisted domain and every live entry actually reaches a reader, and fails on its own after six months unless a person re-verifies. Doctrine in [`product/community-continuity.md`](product/community-continuity.md) |
...
| 10.b | The accountability log has a retrieval path | **deferred, and its reasoning was stale** | Trigger: *a reviewer is named.* **The original justification is no longer true and said so for months:** it read *"`merkle.ts` has exactly one consumer — its own test file. Nothing writes an entry."* Both halves were already contradicted by this same document — sprints B/C/D are done above, `review.ts` imports `verifyInclusion`, `roots.ts` exists, `/terminal/log/` ships, and `shared/accountability.ts` writes entries. Found by audit. What survives is the *other* half: 9.8 is unfilled and no night has been carried, so this is still building a pipe for a reviewer who does not exist — deferred on **that**, which is a gate on a person rather than a claim about the code. **Narrowed 2026-09-02:** the deadlock in that gate was named and half-broken. 9.8 waited on a reviewer, and a reviewer waited on the job being worth taking; `--review` settles the second half without waiting for the first, and reads the log from disk on the box. What is still deferred is the *remote* retrieval path — a reviewer who is not the Stationkeeper, on a machine that is not the box — and that one really does gate on a named person, because until there is one there is nobody to hand a credential to |
...
**Mostly not code**, which is the finding rather than an excuse — but not a place to stop, either. Every item here is a person who has not appeared, and the reason is rarely that nobody cares: it is that the job as it stands is unperformable, or unprovable, or has no visible edge. 9.6 was *human* until `watchtower-daemon --check` gave a second Stationkeeper a way to prove their box was visible; 9.8 was *human* until `--review` made "minutes per week" a true statement. Both are still human. **The code cannot fill these roles, and it can stop being the reason they are empty** — that is the work these items ask for, and it is the half that was going undone while the column said `human`.
...
| P2 | ~~`WHY` everywhere~~ | **withdrawn** | Attempted, measured, abandoned. The transform produced 96 disclosures and **31 browser tests went red**, each asserting a now-hidden sentence must be visible — because `.cost` is `color: faint; font-size: .93rem` and nothing more. **It is a typographic class, not a semantic one**, carrying asides and primary state alike, so nothing mechanical can separate what may be hidden from what may not. Prose relocation folds into P3 as a per-screen judgement. See `design/panel.md` |
| P3 | ~~The readouts~~ | **done** | **Watch and sign-on done.** Watch: the 218-word preamble is two visible sentences and a `Why`, state is slots, the empty board reads `NO CONTACT`, per-page CSS 68→28 lines. Sign-on reads the receipt through the same `Panel` and the same `pageableNow` call as status, so two screens cannot drift on one fact. **Distress is reclassified to P4/P6** — its prose is instruction, and instruction stays visible; what it needs is `Elapsed` and the ladder. Standing is converted: the model's doctrine went behind `Why`, the cost an operator acts on stayed visible, and each held endorsement is one readout carrying scope, endorser and age together |
| P4 | ~~Motion that carries state~~ | **done** | `Window`, `Elapsed` and `Board` are live on the watch: every ask carries a real depleting bar from `RESPONSE_WINDOW`, and `distress: null` routes to a bar that climbs and never arrives. Three findings — an animation with no duration **filled to its end state**, rendering a window as fully run out, so it is now gated on `data-run`; the board timestamps asks by **receipt, not `created_at`**, which made the obvious test unreachable; and `test.use({ reducedMotion })` never reached the page, so a whole file would have measured the ordinary branch while claiming otherwise |
